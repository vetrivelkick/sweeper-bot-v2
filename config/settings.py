...
def net_edge_per_share(buy_price, loser_price, gas_per_share=0.0001, is_maker=False):
    gross = 1.0 - buy_price
...
class SweeperConfig:
    ...
    def validate(self):
        if not (0.90 <= self.buy_price <= 0.999):
            return False
        if self.max_daily_loss <= 0 or self.gas_floor <= 0:
            return False
        return True
...
class BotState:
    ...
    def to_dict(self):
        return {
            "worked_markets": list(self.worked_markets),
    ...
    def from_dict(cls, d):
        return cls(
            worked_markets=set(d.get("worked_markets", [])),
            ...
            total_redeems=d.get("total_redeems", 0),
            total_merges=d.get("total_merges", 0),
            total_recycled=d.get("total_recycled", 0.0),
            open_orders=d.get("open_orders", []),
